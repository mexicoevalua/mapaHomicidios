# Crear mapa interactivo con la tasa de averiguaciones para cualquier delito
...
# Total crimenes
sum(data$count, na.rm=T) #6,419,060
...
other  <- c("DELITOS PATRIMONIALES","DELITOS SEXUALES \\(VIOLACION\\)","LESIONES", "OTROS DELITOS")
...
#----------HOMICDIOS TOTALES
aveHomTot <- ddply(data, c("state_code","mun_code","year","group"), summarize,
             averiguaciones = sum(count,na.rm=T))


# Mantener solo total de homicidios 
table(aveHomTot$group)
aveHomTot <- subset(aveHomTot, aveHomTot$group == "HOMICIDIOS")
aveHomTot$group  <- gsub(pattern="HOMICIDIOS", "TOTAL DE HOMICIDIOS",aveHomTot$group)
format(table(aveHomTot$group)  / length(aveHomTot$group)*100, digits=2)
#====
...
# Calcular tasa del total de homicidios
aveHomTot  <- merge(aveHomTot, population, by= c("state_code","mun_code","year"))
aveHomTot$rate  <- (aveHomTot$averiguaciones*100000 / aveHomTot$population)
summary(aveHomTot$rate)

# Cambiar formato % en rate para reducir a dos decimales
aveHomTot$rate  <- as.numeric(format(round(as.numeric(aveHomTot$rate),2), nsmall = 2))
...
# Agregar abreviaturas de los estados
aveHomTot  <- merge(aveHomTot,temp)


# Agregar el total de delitos al archivo de grupos
aveTot$group  <- "Total"
aveHomTot$group <- "Homicidios Total"
head(ave)
head(aveTot)
head(aveHomTot)
aveTot  <- aveTot[,c(1,2,3,9,4:8)]
names(ave) == names(aveTot) # Las columnas coinciden
names(ave) == names(aveHomTot) # Las columnas coinciden
ave  <- rbind(ave, aveTot, aveHomTot)
names(ave)
...
n  <- names(tasas_ave)
n  <- gsub("_rate_","",n)
n  <- gsub("HOMICIDIOS DOLOSOS","HOD",n)
n  <- gsub("Homicidios Total","HOT",n)
...
require(foreign)
shp  <- read.dbf("shapefiles//MapaDelitos.dbf", as.is=T)
...
write.dbf(shp,"shapefiles//MapaDelitos.dbf")
...
fillKey  <- subset(tasas_ave$TOTHOM2014, !is.na(tasas_ave$TOTHOM2014))
...
# Tasa de homicidios
fillKey  <- subset(tasas_ave$HOMDOL2014, !is.na(tasas_ave$HOMDOL2014))
fillKey  <- cut(unique(fillKey),quantile(unique(fillKey), probs = seq(0, 1, by = 0.20), na.rm=TRUE), dig.lab = 5, include.lowest=T, right=F)
table(fillKey)

# Tasa de secuestros
fillKey  <- subset(tasas_ave$SEC2014, !is.na(tasas_ave$SEC2014))
fillKey  <- cut(unique(fillKey),quantile(unique(fillKey), probs = seq(0, 1, by = 0.20), na.rm=TRUE), dig.lab = 5, include.lowest=T, right=F)
table(fillKey)

# Tasa de robos
fillKey  <- subset(tasas_ave$ROB2014, !is.na(tasas_ave$ROB2014))
fillKey  <- cut(unique(fillKey),quantile(unique(fillKey), probs = seq(0, 1, by = 0.20), na.rm=TRUE), dig.lab = 5, include.lowest=T, right=F)
table(fillKey)

# Tasa de otros delitos
fillKey  <- subset(tasas_ave$OTR2014, !is.na(tasas_ave$OTR2014))
fillKey  <- cut(unique(fillKey),quantile(unique(fillKey), probs = seq(0, 1, by = 0.20), na.rm=TRUE), dig.lab = 5, include.lowest=T, right=F)
table(fillKey)

fillKey  <- subset(tasas_ave$TOT2014, !is.na(tasas_ave$TOT2014))
fillKey  <- cut(unique(fillKey),quantile(unique(fillKey), probs = seq(0, 1, by = 0.20), na.rm=TRUE), dig.lab = 5, include.lowest=T, right=F)
table(fillKey)
### Al final sólo se expuso el mapa con la tasa de homicidios por cada 100 mil habitantesç
# para cada municipio en 2014
